...
class CollectReport:
	...
	active_user_dict={}
		...
		traceback.print_exc()
	count=len(shop_list)
	header="%s日订购用户,且服务已经开始用户数:%s\n" %(self.order_date,count)
	print header
	for date_type,active_count in active_user_dict.iteritems():
	    header+="%s天活跃数:%s 比例:%s" %(date_type,active_count,active_count/float(count))	
...
if __name__ == '__main__':
    d1=datetime.date(2013,8,1)
    d2=datetime.date(2013,8,16)
    data=[]
    while d1< d2:
	syb=CollectSYBReport(d1)
	shop_list=syb.get_shop_list()
	d1 += datetime.timedelta(1)
        #print len(shop_list)
        active_user_dict=syb.collect_report(shop_list)
	data.append(active_user_dict)
    for d in data:
	print d
